...
// Heater controls:
// R26 = HEATER1, controlled by the original HB_EN signal
// R32 = HEATER2, controlled by the original LB_EN signal
const int PIN_HEATER1_EN = 27;
const int PIN_HEATER2_EN = 13;

// Onboard LED: GPIO18 -> 2.2k ohm -> LED -> GND
const int PIN_LED = 18;

// Cell-voltage differential dividers:
// CH1 = (B+ - BM) / 4, shown as B1.
// CH2 = (BM - B-) / 4, shown as B2.
// 75k/25k -> divider ratio 4.0.
// If the 75k resistors were changed to 82k, use 4.28.
const float CELL1_DIVIDER = 4.0;
const float CELL2_DIVIDER = 4.0;
...
const char *WIFI_AP_PASSWORD = "elec3117";
...
void allPowerOutputsOff() {
  ...
  digitalWrite(PIN_HEATER1_EN, LOW);
  digitalWrite(PIN_HEATER2_EN, LOW);
}
...
String toggleOutput(const char *name, int pin) {
  bool newState = !digitalRead(pin);
  digitalWrite(pin, newState ? HIGH : LOW);

  String result = String(name);
  result += newState ? " is ON." : " is OFF.";
  return result;
}
...
String gpioStateText() {
  ...
  result += "\nHEATER1 R26 GPIO27: ";
  result += digitalRead(PIN_HEATER1_EN) ? "ON" : "OFF";
  result += "\nHEATER2 R32 GPIO13: ";
  result += digitalRead(PIN_HEATER2_EN) ? "ON" : "OFF";
  result += "\nLED GPIO18:         ";
...
String mcpReadingText(const MCPReading &reading) {
  ...
  bool b1Valid = reading.valid[0];
  bool b2Valid = reading.valid[1];

  result += "\n----- CELL VOLTAGES -----\n";
  result += "B1 (CH1 x ";
  result += String(CELL1_DIVIDER, 2);
  result += "): ";
  result += b1Valid
    ? String(reading.voltage[0] * CELL1_DIVIDER, 3) + " V"
    : "read failed";
  result += "\nB2 (CH2 x ";
  result += String(CELL2_DIVIDER, 2);
  result += "): ";
  result += b2Valid
    ? String(reading.voltage[1] * CELL2_DIVIDER, 3) + " V"
    : "read failed";

  if (b1Valid && b2Valid) {
    float b1Voltage = reading.voltage[0] * CELL1_DIVIDER;
    float b2Voltage = reading.voltage[1] * CELL2_DIVIDER;
    result += "\nPack (B1 + B2): ";
    result += String(b1Voltage + b2Voltage, 3);
    result += " V\nCell difference: ";
    result += String(fabs(b1Voltage - b2Voltage), 3);
    result += " V";
  }
...
String menuText() {
  ...
  result += "r : read INA219, B1/B2, MCP3424 and temperatures\n";
  ...
  result += "1 : toggle HEATER1 (R26 / GPIO27)\n";
  result += "2 : toggle HEATER2 (R32 / GPIO13)\n";
...
<main>
  ...
  <section class="grid">
    <div class="card">
      <h2>B1 cell voltage · MCP3424 CH1</h2>
      <div class="value" id="b1">-- V</div>
      <div class="small" id="ch1">CH1 ADC: -- V · ×4.00</div>
    </div>
    <div class="card">
      <h2>B2 cell voltage · MCP3424 CH2</h2>
      <div class="value" id="b2">-- V</div>
      <div class="small" id="ch2">CH2 ADC: -- V · ×4.00</div>
    </div>
    <div class="card">
      <h2>Battery pack</h2>
      <div class="value" id="pack">-- V</div>
      <div class="small" id="cellDiff">B1/B2 difference: -- V</div>
    </div>
    <div class="card">
  ...
  <section class="card" style="margin-bottom:12px">
    ...
    <div class="states">
      ...
      <div class="state" id="stHeater1">Heater 1<br><b>--</b></div>
      <div class="state" id="stHeater2">Heater 2<br><b>--</b></div>
  ...
  <section class="card" style="margin-bottom:12px">
    ...
    <div class="buttons">
      <button class="primary" onclick="run('r')">r · Read all data</button>
      ...
      <button class="warn" onclick="confirmRun('1','Toggle HEATER1? Keep watching its temperature.')">1 · Toggle HEATER1</button>
      <button class="warn" onclick="confirmRun('2','Toggle HEATER2? Keep watching its temperature.')">2 · Toggle HEATER2</button>
...
<script>
  ...
  async function refresh() {
    ...
    try {
      ...
      $("b1").textContent = fmt(data.battery.b1_v, 3, " V");
      $("b2").textContent = fmt(data.battery.b2_v, 3, " V");
      $("pack").textContent = fmt(data.battery.pack_v, 3, " V");
      $("cellDiff").textContent =
        "B1/B2 difference: " + fmt(data.battery.difference_v, 3, " V");
      $("ch1").textContent =
        "CH1 ADC: " + fmt(data.mcp.ch1_v, 5, " V") +
        " · ×" + data.battery.b1_divider.toFixed(2);
      $("ch2").textContent =
        "CH2 ADC: " + fmt(data.mcp.ch2_v, 5, " V") +
        " · ×" + data.battery.b2_divider.toFixed(2);
      ...
      setState("stHeater1", data.gpio.heater1);
      setState("stHeater2", data.gpio.heater2);
...
String buildStatusJson() {
  INAReading ina = getINA219Reading();
  MCPReading mcp = getMCP3424Reading();
  bool b1Valid = mcp.found && mcp.valid[0];
  bool b2Valid = mcp.found && mcp.valid[1];
  float b1Voltage = b1Valid
    ? mcp.voltage[0] * CELL1_DIVIDER
    : NAN;
  float b2Voltage = b2Valid
    ? mcp.voltage[1] * CELL2_DIVIDER
    : NAN;
  float packVoltage = (b1Valid && b2Valid)
    ? b1Voltage + b2Voltage
    : NAN;
  float cellDifference = (b1Valid && b2Valid)
    ? fabs(b1Voltage - b2Voltage)
    : NAN;
  ...
  json += "\"battery\":{\"b1_valid\":";
  json += b1Valid ? "true" : "false";
  json += ",\"b2_valid\":";
  json += b2Valid ? "true" : "false";
  json += ",\"b1_divider\":";
  json += jsonNumber(CELL1_DIVIDER, 2);
  json += ",\"b2_divider\":";
  json += jsonNumber(CELL2_DIVIDER, 2);
  json += ",\"b1_v\":";
  json += b1Valid ? jsonNumber(b1Voltage, 3) : "null";
  json += ",\"b2_v\":";
  json += b2Valid ? jsonNumber(b2Voltage, 3) : "null";
  json += ",\"pack_v\":";
  json += (b1Valid && b2Valid)
    ? jsonNumber(packVoltage, 3)
    : "null";
  json += ",\"difference_v\":";
  json += (b1Valid && b2Valid)
    ? jsonNumber(cellDifference, 3)
    : "null";
  json += "},";
  ...
  json += ",\"heater1\":" + String(digitalRead(PIN_HEATER1_EN));
  json += ",\"heater2\":" + String(digitalRead(PIN_HEATER2_EN));
